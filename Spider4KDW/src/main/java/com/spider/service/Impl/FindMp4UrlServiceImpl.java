...
@Service
public class FindMp4UrlServiceImpl implements FindMp4UrlService, PageProcessor {

	@Resource
	private Mp4UrlPipline mp4UrlPiplin;

	// 部分一：抓取网站的相关配置，包括编码、抓取间隔、重试次数等
	private Site site = Site.me().setDomain("http://www.cao7000.com/")
			.setUserAgent(
					"Mozilla/5.0 (Macintosh; Intel Mac OS X 10_7_2) AppleWebKit/537.31 (KHTML, like Gecko) Chrome/26.0.1410.65 Safari/537.31")
			.setRetryTimes(3).setSleepTime(1);
	...
	public void test(String url) {
		Spider.create(new FindMp4UrlServiceImpl()).addPipeline(mp4UrlPiplin).addUrl(url)
				// 开启5个线程抓取
				.thread(5)
				// 启动爬虫
				.run();
